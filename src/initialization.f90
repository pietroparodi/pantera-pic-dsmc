...
MODULE initialization
   ...
   SUBROUTINE DEF_BOUNDARY_CONDITION_SPECIES(DEFINITION)
      ...
      ELSE IF (STRARRAY(3) == 'moving') THEN
         GRID_BC(IPG)%PARTICLE_BC(ISP) = MOVING
         READ(STRARRAY(4), '(ES14.0)') GRID_BC(IPG)%WALL_TEMP
         READ(STRARRAY(5), '(ES14.0)') GRID_BC(IPG)%U_PISTON(1)
         READ(STRARRAY(6), '(ES14.0)') GRID_BC(IPG)%U_PISTON(2)
         READ(STRARRAY(7), '(ES14.0)') GRID_BC(IPG)%U_PISTON(3)
